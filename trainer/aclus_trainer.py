...
class ACLUSTrainer:
    def __init__(self, config):
        ...
        self.forward_loss = InfoNCECriterion(config)
        self.forward_loss_com = torch.nn.MSELoss()
        ...
        if self.enable_wandb:
            ...
            if self.gpu==0 or self.log_all:
                wandb.init(project="ACLUS",name = save_dir+'_gpu_'+str(self.rank))
    ...
    def construct_model(self):
        ...
        assert self.stage == 'train', ValueError(f'Invalid stage: {self.stage}, only "train" for ACLUS training')
    ...
    def train_epoch(self, epoch, printer=print):
        ...
        images, com_images, masks = prefetcher.next()
        i = 0
        while images is not None:
            ...
            # forward
            tflag = time.time()
            result = self.model(view1, view2, self.mm, masks.to('cuda'))
            forward_time.update(time.time() - tflag)

            # loss and backward
            tflag = time.time()
            loss = 0
            result, result_com = self.model(view1, view2, com_images, self.mm, masks.to('cuda'))
            for q, target_z, pinds, tinds in result:
                loss += self.forward_loss(target_z, q, tinds.to('cuda'), pinds.to('cuda'))
                # print(loss)
            for q_com, target_z_com in result_com:
                loss += self.forward_loss_com(target_z_com, q_com)
        ...
        if self.enable_wandb:
            ...
            if self.gpu == 0 or self.log_all:
                ...
                print(f"Epoch: {epoch}")
                print(f"Average Loss (Per-Epoch): {round(loss_meter.avg, 5)}")
                print(f"Average Batch-Time (Per-Epoch): {round(batch_time.avg, 5)}")
                print(f"Average Data-Time (Per-Epoch): {round(data_time.avg, 5)}")
                print(f"Average Forward-Time (Per-Epoch): {round(forward_time.avg, 5)}")
                print(f"Average Backward-Time (Per Epoch): {round(backward_time.avg, 5)}")
